...
class BinanceClientFactory(WebSocketClientFactory, BinanceReconnectingClientFactory):
    ...
    _connection_lost_payload = {
        'e': 'connection_lost',
        'm': 'The connection has been lost'
    }
    _connection_started_payload = {
        'e': 'connection_started',
        'm': 'The connection is started'
    }

    def clientConnectionFailed(self, connector, reason):
        print('Connection failed. Reason: {}'.format(reason))
        BinanceReconnectingClientFactory.clientConnectionFailed(self, connector, reason)
    ...
    def clientConnectionLost(self, connector, reason):
        ...
        BinanceReconnectingClientFactory.clientConnectionLost(self, connector, reason)

        # self.retry(connector)
        # if self.retries == 1:
        #     self.callback(self._connection_lost_payload)
        # if self.retries > self.maxRetries:
        #     self.callback(self._reconnect_error_payload)

    def startedConnecting(self, connector):
        print('Try to start connection...')

    def connected(self):
        print('Start connection (connected)!')
        self.callback(self._connection_started_payload)
